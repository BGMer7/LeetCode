...
{
public:
    void printVectorinVector(vector<vector<int>> &vector)
    {
        for (int i = 0; i < vector.size(); i++)
        {
            cout << "["<< vector[i][0] << "," << vector[i][1] << "] ";
        }
        cout << endl;
    }
};
...
int main()
{
    ...
    a.assign(testCase.begin(), testCase.end());
    b.assign(testCase.begin(), testCase.end());
    sort(a.begin(), a.end());
    sort(b.begin(), b.end(), [](vector<int> v1, vector<int> v2) {
        return v1[1] < v2[1];
    }); // 以结尾的区间作为排序
    Print p;
    p.printVectorinVector(a);
    p.printVectorinVector(b);
}